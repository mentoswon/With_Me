...
// 	function checkBirth() {
// 		// 입력받은 전화번호 값 가져오기
// 		let mem_birthday = $("#mem_birthday").val();
		
// 		let regex = /^([0-9]{2}(0[1-9]|1[0-2])(0[1-9]|[1,2][0-9]|3[0,1]))$/;

// 		// 정규표현식 문자열을 관리하는 객체(regex)의 exec() 메서드 호출하여
// 		// 검사할 문자열을 전달하면 정규표현식 일치 여부 확인 가능
// 		if(!regex.exec(mem_birthday)) { // 불일치
// 			checkBirthResult = false; // 검사 적합 여부 false(부적합) 값 저장
// 		} else { // 일치
// 			checkBirthResult = true; // 검사 적합 여부 true(적합) 값 저장
// 		}
