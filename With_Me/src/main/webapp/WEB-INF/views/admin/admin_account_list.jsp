...
<!-- 										</td> -->
				...
				<div id="pageList">
					...
					<input type="button" onclick="location.href='AdminAccountList?pageNum=${pageNum - 1}&status=${param.status}'" <c:if test="${pageNum <= 1}">disabled</c:if>>
					...
					<input type="button" onclick="location.href='AdminAccountList?pageNum=${pageNum + 1}&status=${param.status}'" <c:if test="${pageNum >= pageInfo.maxPage}">disabled</c:if>>
				</div>
			</article>
		</section>
	</div>
