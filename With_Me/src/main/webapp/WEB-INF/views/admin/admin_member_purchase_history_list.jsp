...
<script>
	...
	function showListLimit(limit){
		location.href="PurchaseHistoryList?mem_email=${member.mem_email}&listLimit=" + limit;
	}
</script>
</head>
<body>
		...
		<section class="wrapper">
			...
			<article class="main">
				...
				<div class="wrapper_top">
					...
					<form action="PurchaseHistoryList">
						<div class="search">
							<span>Search</span>
					...
					<table border="1">
						...
						<c:forEach var="PHL" items="${purchaseHistorylist}">
							<tr align="center">
								...
								<td>
									<c:choose>
										<c:when test="${PHL.order_pay_status eq '미결제'}">
											아직 결제가 진행되지 않았습니다
										</c:when>
										<c:otherwise>
											<%-- 
											...
											<fmt:formatDate value="${order_pay_date}" pattern="yyyy-MM-dd HH:mm"/>
										</c:otherwise>
									</c:choose>
								</td>
						...
						<c:if test="${empty purchaseHistorylist}">
							<tr>
								<td align="center" colspan="6">구매내역이 없습니다.</td>
							</tr>
						</c:if>
					</table>
				</div>
				<%-- ========================== 페이징 처리 영역 ========================== --%>
				<c:if test="${not empty purchaseHistorylist}">
					<div id="pageList">
						...
						<input type="button" onclick="location.href='PurchaseHistoryList?mem_email=${member.mem_email}&pageNum=${pageNum - 1}'" <c:if test="${pageNum <= 1}">disabled</c:if>>
						...
						<%-- [다음] 버튼 클릭 시 PurchaseHistoryList 서블릿 요청(파라미터 : 현재 페이지번호 + 1) --%>
						...
						<input type="button" onclick="location.href='PurchaseHistoryList?mem_email=${member.mem_email}&pageNum=${pageNum + 1}'" <c:if test="${pageNum >= pageInfo.maxPage}">disabled</c:if>>
					</div>
				</c:if>
			</article>
		</section>
