	...
	<div class="inner">
		<section class="wrapper">
			...
			<article class="main">
				...
				<%-- ========================== 페이징 처리 영역 ========================== --%>
				<c:if test="${not empty sponsorshipHistoryList}">
					<div id="pageList">
						...
						<input type="button" onclick="location.href='SponsorshipHistoryList?mem_email=${member.mem_email}&pageNum=${pageNum - 1}'" <c:if test="${pageNum <= 1}">disabled</c:if>>
						...
						<input type="button" onclick="location.href='SponsorshipHistoryList?mem_email=${member.mem_email}&pageNum=${pageNum + 1}'" <c:if test="${pageNum >= pageInfo.maxPage}">disabled</c:if>>
					</div>
				</c:if>
			</article>
		</section>
